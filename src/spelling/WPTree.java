...
public class WPTree implements WordPath {
  ...
  public WPTree() {
    ...
    DictionaryLoader.loadDictionary(d, "/Users/mateusz/IdeaProjects/MOOCTextEditor/data/dict.txt");
  ...
  public List<String> findPath(String word1, String word2) {
    ...
    for each n in the list of neighbors
     if n is not visited
       ...
       add the child node for n to the back of the queue
       if n is word2
    ...
    boolean found = false;
    int count = 0; // A counter to break the loop below at some point for impossible paths
    ...
    while (!queue.isEmpty() && !found) {
      ...
      List<String> currentDistanceOne = nw.distanceOne(current.getWord(), true);
      for (String string : currentDistanceOne) {
        if (!visited.contains(string)) {
          WPTreeNode child = current.addChild(string);
          visited.add(string);
          queue.add(child);
          if (string.equals(word2)) {
